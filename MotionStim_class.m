...
%                     thresh = (mean(APT.SMOO(APT.AP))*5)/12;
%                     if any(APT.SMOO(APT.AP)<thresh)
%                         tRM = find(APT.SMOO(APT.AP)<thresh);
%                         APT.AP(tRM) = [];
%                     end
                    if any(isoutlier(APT.SMOO(APT.AP),'percentiles',[2 100]))
                        tRM = isoutlier(APT.SMOO(APT.AP),'percentiles',[2 100]);
                        APT.AP(tRM) = [];
                    end
                    ...
                    if any(isoutlier(t,'percentiles',[0 99]))
                        ...
                        toRM = [];
                        for ii = 1:length(IDS)
                            if APT.SMOO(APT.AP(IDS(ii)))<mean(APT.SMOO(APT.AP))*(3/4)
                                toRM = [toRM IDS(ii)];
                            elseif APT.SMOO(APT.AP(IDS(ii)+1))<mean(APT.SMOO(APT.AP))*(3/4)
                                toRM = [toRM IDS(ii)+1];
                            end
                        end
                        if any(isinf(t))
