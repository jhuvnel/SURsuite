classdef ActionPotential_class <handle
    ...
    methods
        ...
        function [APT_unzeroed, APT] = SimpleFindAPs(obj, DAT, THRESH)
            % Subtract out baseline offset.
            if isrow(DAT)
                DAT = DAT';
            end
            ...
            MM_unzeroed = movmax(DAT, 10000);
            THRESH_unzeroed = 0.57*MM_unzeroed;

            MM = movmax(DAT, 10000);
            THRESH = 0.57*MM;

            % "Threshold" based on moving maximum.
            HIT_unzeroed = DAT_unzeroed > THRESH_unzeroed;
            ...
            % Find the "clumps" of data points that are above the
            % threshold. Each Clump may be an action potential.
            CLUMP_idx_unzeroed = [diff(HIT_idx_unzeroed); max(HIT_idx_unzeroed)*10] > 100;
            CLUMP_unzeroed = HIT_idx_unzeroed(CLUMP_idx_unzeroed);

            CLUMP_idx = [diff(HIT_idx); max(HIT_idx)*10] > 100;
            CLUMP = HIT_idx(CLUMP_idx);
            ...
            for i=1:length(CLUMP)
                if i == 1
                    bd = HIT_idx(find(HIT_idx<=CLUMP(i)));
                else
                    bd = HIT_idx(find(HIT_idx>CLUMP(i-1) & HIT_idx<=CLUMP(i)));
                end
                if bd(end) > length(DAT)
                    bd = 1+CLUMP(i):length(DAT);
                end
                vals = DAT(bd);
                apidx = find(vals == max(vals));
                apidx = apidx(1);
            ...
            for i=1:length(CLUMP_unzeroed)
                if i == 1
                    bd = HIT_idx_unzeroed(find(HIT_idx_unzeroed<=CLUMP_unzeroed(i)));
                else
                    bd = HIT_idx_unzeroed(find(HIT_idx_unzeroed>CLUMP_unzeroed(i-1) & HIT_idx_unzeroed<=CLUMP_unzeroed(i)));
                end
                vals_unzeroed = DAT_unzeroed(bd);
                apidx_unzeroed = find(vals_unzeroed == max(vals_unzeroed));
                apidx_unzeroed = apidx_unzeroed(1);
                AP_unzeroed(i) = bd(apidx_unzeroed);
            end

            % % Get rid of any low-amplitude outliers, whose peak is well
            % % below the mean of all the peaks.
            % AP_unzeroed(DAT_unzeroed(AP_unzeroed) < 0.35*mean(DAT_unzeroed(AP_unzeroed))) = [];
            % AP_unzeroed(DAT_unzeroed(AP_unzeroed) > 3*mean(DAT_unzeroed(AP_unzeroed))) = [];
            % AP(DAT(AP) < 0.35*mean(DAT(AP))) = [];
            % AP(DAT(AP) > 3*mean(DAT(AP))) = [];

            

            if any(isoutlier(DAT(AP),'percentiles',[0.5 100]))
                tRM = isoutlier(DAT(AP),'percentiles',[0.5 100]);
                AP(tRM) = [];
            end

            AP_unzeroed(DAT_unzeroed(AP_unzeroed) < 0.7*mean(DAT_unzeroed(AP_unzeroed))) = [];
            AP_unzeroed(DAT_unzeroed(AP_unzeroed) > 3*mean(DAT_unzeroed(AP_unzeroed))) = [];
            AP(DAT(AP) < 0.7*mean(DAT(AP))) = [];
            AP(DAT(AP) > 3*mean(DAT(AP))) = [];

            % apT = (0:(length(DAT)-1))/200000;
            % t = 1./diff(apT(AP));
            % if any(isoutlier(t,'percentiles',[0 99]))
            %     IDS = find(isoutlier(t,'percentiles',[0 99]));
            %     toRM = [];
            %     for ii = 1:length(IDS)
            %         if DAT(AP(IDS(ii)))<mean(DAT(AP))*(3/4)
            %             toRM = [toRM IDS(ii)];
            %         elseif DAT(AP(IDS(ii)+1))<mean(DAT(AP))*(3/4)
            %             toRM = [toRM IDS(ii)+1];
            %         end
            %     end
            %     if any(isinf(t))
            ...
            % Return a bunch of internal values in a struct, for debugging
            % or plotting.
            APT_unzeroed = MakeStruct(CLUMP_unzeroed, AP_unzeroed, SMOO_unzeroed, AVG_unzeroed, AVG_RAW_unzeroed);
            APT = MakeStruct(CLUMP, AP, SMOO, AVG, AVG_RAW);
        ...
        function [samps, time, startPt, endPt, peakPolarization1, peakPolarization2, maxHyperpol] = findAPWidth(obj, apTemp, chan, debugPlot, debugPlots)
            if isprop(obj.DAT,'APRefProperties')
                if obj.DAT.APRefProperties.(chan).peakDePol2 == 0
                    chanUsed = obj.DAT.GetChan(chan);
                    ap = apTemp.AVG_RAW;
                    apfilt = smooth(ap,15,'sgolay');
                    if debugPlot
                        startPt=obj.DAT.APRefProperties.(chan).startPt;

                        pt = obj.DAT.APRefProperties.(chan).peakDePol1;
                        [~,ml]=max(ap(pt-5:pt+5));
                        peakPolarization1 = ml+pt-6;
                        peakPolarization2 = 0;

                        mt = obj.DAT.APRefProperties.(chan).maxHyperpol;
        ...
        function findAPSAfterSubtraction(obj, idxss, chan, app, debugFlg, findFlg)
            ...
            while id2U <= length(idxss)
                ...
                if app.APThreshChange
                    mm(:) = app.FindAPThresholdEditField.Value;
                end
                ...
                if mAP > app.DAT.APRefProperties.([chan]).peakDePol1Val*2
                    mAP = app.DAT.APRefProperties.([chan]).peakDePol1Val;
                end
                skipNext = 0;
                for ii = 1:sz(1)
                    if ~skipNext
                ...
                if isempty(app)
                    ...
                    ALLSTIMPlot = reshape(D.StimBlocks(idxs).([chan,'_ALLSTIM'])',1,198000);
                    ZO = reshape(D.StimBlocks(idxs).([chan,'_ZAPPED'])',1,198000)-mean(reshape(D.StimBlocks(idxs).([chan,'_ZAPPED'])',1,198000));
...
%                     aa = axes(app.AnalysisPlotsTab);
                    ...
                    aa.YLim = [min(ZO(find(apl)))*-.8 max(ZO(find(apl)))*1.3];
...
%                     aa.Title.String(name);
                    ...
                    D.StimBlocks(idxs).([chan,'_APCount']) = app.tempAPCount;%%%%%%%%%%% Need to check this against APLoc data
                    obj.DAT = D;
                ...
                if ~app.APThreshChange
                    id2U = id2U + 1;
                    app.FindAPThresholdEditField.Value = 0;
                end
            end
            if isempty(app)
